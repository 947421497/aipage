...
> **文档用途**: 供 AI 理解产品需求并指导开发实现
> **合并说明**: 基于 PRD v2.0（含49项修订）与开发指南合并，冲突处以 PRD 为准
...
| 提示词模板 | 指导 AI 生成内容的预设指令模板 |
| Cron 任务 | 定时执行的自动化任务，如批量生成、百度推送等 |
...
│                    后台管理 (admin)                   │
├──────────┬──────────┬───────────┬────────────────────┤
│ AI引擎    │ 关键词管理 │ 页面管理   │ 定时任务            │
│ AiConfig │ Keyword  │ Page      │ Task               │
│ Prompt   │          │           │                    │
├──────────┴──────────┴───────────┴────────────────────┤
│                    公共层 (common)                     │
├─────────────────────┬───────────────────────────────┤
│  ai_chat()          │  Keyword Model                 │
│  filter_landing_html│  Page Model                    │
│  to_pinyin()        │  Task Model                    │
│  generate_url_path()│  TaskLog Model                 │
│  baidu_push()       │                                │
│  parse_seo_meta()   │                                │
│  render_prompt()    │                                │
├─────────────────────┴───────────────────────────────┤
│                    前台 (index)                       │
├─────────────────────┬───────────────────────────────┤
│  Site::dispatch()   │  Cron::run()                   │
│  Site::index()      │  Index::index()                │
└─────────────────────┴───────────────────────────────┘
```
...
| 密钥管理 | 新增时必填（Ollama 除外），更新时若提交空值则保留原值；加密存储，API 响应脱敏显示 |
| 连接测试 | 提供测试连接功能，30 秒内不可重复测试（通过缓存记录上次测试时间） |
...
| 激活机制 | 同类型同方向下仅一个模板可激活，激活时自动取消同组其他模板 |
...
| 功能模块 | 需求描述 | 优先级 |
|----------|----------|--------|
| 关键词 CRUD | 支持关键词的增删改查，按词/来源/是否有页面筛选 | P0 |
| 删除限制 | 仅允许删除已停用(status=0)且未生成页面的关键词 | P0 |
| 批量启停 | 支持批量启用/停用关键词 | P0 |
| CSV 导入 | 支持上传 CSV 文件批量导入，限制 2MB / 1000 行 | P1 |
| CSV 导出 | 支持导出全部关键词数据 | P1 |
| AI 拓词 | 选择拓词方向 → AI 生成候选词 → 标记已存在词 → 勾选导入 | P1 |
| 批量生成 | 选择多个关键词 → flock 互斥执行 → 前端 3 秒轮询进度 | P1 |
...
| 功能模块 | 需求描述 | 优先级 |
|----------|----------|--------|
| 页面列表 | 关联显示关键词，支持按标题/状态筛选 | P0 |
| 页面编辑 | 上方表单编辑 + 下方 textarea 代码编辑区（等宽字体，最小高度 500px）；已发布页面的 url_path 只读 | P0 |
| AI 重写 | 调用 AI 重新生成内容，**重写前弹出确认框**，重写后状态回退为草稿，推送标记重置 | P1 |
| 状态切换 | 发布时重置推送标记；下线后页面不可访问，前台返回 404 | P0 |
| 缓存管理 | 页面增删改时自动清除前台缓存 | P0 |

### 4.4 定时任务管理

| 功能模块 | 需求描述 | 优先级 |
|----------|----------|--------|
| 任务列表 | 显示任务名称、执行频率、最后执行状态、累计统计 | P0 |
| 任务配置 | 支持编辑任务名称、执行频率描述、启用状态、超时时间 | P0 |
| 手动执行 | 支持手动触发单个任务执行 | P1 |
| 执行日志 | 查看任务执行历史，失败项红色标记 | P1 |
| 删除限制 | 存在执行日志的任务不可删除 | P0 |
| 并发控制 | 自动检测是否有执行中的日志，防止并发执行 | P0 |
| 超时恢复 | 每次 Cron 入口检查，将超过 timeout+60秒 仍在执行中的日志标记为失败 | P1 |

**内置任务类型**:

| 任务类型 | 说明 | 触发方式 |
|----------|------|----------|
| generate_page | 批量生成页面（每次最多3个未生成页面的关键词） | Cron |
| push_baidu | 百度普通收录推送（仅推 is_pushed_normal=0 的已发布页面） | Cron |
| push_baidu_fast | 百度快速收录推送（仅推 is_pushed_fast=0 AND is_pushed_normal=0 的新发布页面） | Cron |
| sitemap | 生成 sitemap.xml + robots.txt | Cron / 页面发布后标记脏 |
| clear_cache | 清理缓存 + 清理30天前日志 + 修复 has_page 不一致（result 分段记录） | Cron |

**Cron 触发方式**:
- **HTTP 触发**: `/cron/{key}`，需密钥验证（至少32位随机字符）+ IP 白名单（为空时默认拒绝所有）+ 60秒限频
- **CLI 触发**: `php xphpcli cron:run`，仅限命令行执行

**Sitemap 脏标记策略**: 页面发布/下线/删除时设置缓存 `sitemap_dirty=true`；Sitemap 任务执行时检查该标记，无变更则跳过生成并记录"无变更跳过"，有变更则生成后清除标记。

**批量生成并发控制**: 手动批量生成和 Cron 定时生成共享同一 flock 锁文件。获取锁失败时：手动操作返回提示"已有生成任务执行中，请稍后重试"；Cron 触发记录"因其他生成任务执行中而跳过"并标记为成功。
...
| 功能模块 | 需求描述 | 优先级 |
|----------|----------|--------|
| 路由分发 | `/keyword/{pinyin}.html` → `Site::dispatch()` | P0 |
| 访问控制 | 仅渲染 status=1 的页面，草稿/不存在的页面返回 404 状态码 | P0 |
| 安全防护 | 输出 CSP 头，禁止脚本执行 | P0 |
| Canonical | 每个落地页输出 `<link rel="canonical" href="{绝对URL}">` | P0 |
| 相关推荐 | 页面底部追加"相关推荐"模块（5条同站已发布页面链接） | P1 |
| 访问统计 | 访问时原子递增 view_count | P1 |
| 页面缓存 | 页面缓存 1 小时 | P1 |
...
**HTML 结构包裹**: `filter_landing_html()` 处理后，如果内容不含 `<html>` 标签，自动包裹基本 HTML 结构（含 charset、viewport、canonical、SEO meta 标签）。

**相关推荐实现**: `Site::dispatch()` 渲染时，查询 5 条 status=1 的其他页面，拼成 HTML 块追加到 content 末尾（`</body>` 前或内容末尾）。
...
| 系统信息 | 底部折叠面板，默认收起 | P2 |
| 自动初始化 | 首次访问时自动生成 cron_key（32位随机字符） | P0 |

**系统就绪检查项**:
- AI 配置：至少 1 个 status=1 的配置
- 提示词模板：落地页类型至少 1 个 is_active=1 的模板
...
#### 5.2.1 xphp_ai_config（AI 接入配置）

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | int PK | 是 | 自增 | 主键 |
| name | varchar(50) | 是 | - | 配置名称 |
| api_type | varchar(20) | 是 | - | 接口类型：openai / anthropic / ollama |
| api_url | varchar(500) | 是 | - | API 完整 URL |
| api_key | varchar(255) | 条件 | - | API Key，Ollama 类型非必填 |
| model | varchar(100) | 是 | - | 模型名称 |
| max_tokens | int | 否 | 2000 | 最大 token 数 |
| temperature | decimal(4,2) | 否 | 0.70 | 温度 0.00-2.00 |
| max_retries | tinyint | 否 | 3 | 重试次数 |
| retry_interval | int | 否 | 2 | 重试间隔秒 |
| verify_ssl | tinyint | 否 | 1 | 0 不验证 / 1 验证 SSL（Ollama 厂商预设为 0） |
| call_count | int | 否 | 0 | 累计调用次数（原子递增更新） |
| status | tinyint | 否 | 1 | 0 禁用 / 1 启用 |
| create_time | int | 否 | 0 | 创建时间（框架自动填充） |
| update_time | int | 否 | 0 | 更新时间（框架自动填充） |

**索引**: `idx_ai_config_status (status)`

#### 5.2.2 xphp_prompt（提示词模板）

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | int PK | 是 | 自增 | 主键 |
| name | varchar(50) | 是 | - | 模板名称 |
| type | varchar(20) | 是 | - | 模板类型：page（落地页）/ expand（拓词） |
| direction | varchar(20) | 条件 | - | 拓词方向：related / question / longtail / commercial；落地页类型为空 |
| content | text | 是 | - | 提示词内容，支持变量 |
| is_active | tinyint | 否 | 0 | 0 未激活 / 1 已激活 |
| status | tinyint | 否 | 1 | 0 禁用 / 1 启用 |
| create_time | int | 否 | 0 | 创建时间（框架自动填充） |
| update_time | int | 否 | 0 | 更新时间（框架自动填充） |

**索引**: `idx_prompt_type_active (type, is_active)`

**激活唯一性保障**: 应用层启用新模板时，在同一事务内将同 type+direction 的所有模板 is_active 置为 0，再设置当前模板 is_active=1。

#### 5.2.3 xphp_keyword（关键词）

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | int PK | 是 | 自增 | 主键 |
| word | varchar(100) | 是 | - | 关键词文本（UNIQUE） |
| pinyin | varchar(200) | 是 | - | 拼音（自动生成，用于 URL，UNIQUE） |
| source | varchar(20) | 否 | manual | 来源：manual / ai / csv |
| group_id | int | 否 | NULL | 分组 ID（预留字段，MVP 不建分组表和 UI） |
| status | tinyint | 否 | 1 | 0 停用 / 1 启用 |
| has_page | tinyint | 否 | 0 | 0 无页面 / 1 已有页面（与 page 表事务同步，定时任务兜底校验） |
| create_time | int | 否 | 0 | 创建时间（框架自动填充） |
| update_time | int | 否 | 0 | 更新时间（框架自动填充） |

**索引**: `idx_keyword_status_has_page (status, has_page)`

**pinyin 唯一性**: 拼音冲突时自动追加数字后缀（如 `beijing-2`），由 `generate_url_path()` 函数处理。
...
|------|------|------|--------|------|
| id | int PK | 是 | 自增 | 主键 |
| keyword_id | int | 否 | 0 | 关联关键词 ID，0=无关联 |
...
| ai_config_id | int | 否 | 0 | 使用的 AI 配置 ID，0=未记录 |
| prompt_id | int | 否 | 0 | 使用的提示词 ID，0=未记录 |
| status | tinyint | 否 | 0 | 0 草稿 / 1 已发布 |
| view_count | int | 否 | 0 | 访问量（原子递增） |
| is_pushed_normal | tinyint | 否 | 0 | 0 未推送 / 1 已推送百度普通收录 |
| is_pushed_fast | tinyint | 否 | 0 | 0 未推送 / 1 已推送百度快速收录 |
| create_time | int | 否 | 0 | 创建时间（框架自动填充） |
| update_time | int | 否 | 0 | 更新时间（框架自动填充） |

**索引**: `idx_page_keyword_id (keyword_id)`, `idx_page_status_pushed (status, is_pushed_normal)`

**url_path 存储格式**: 仅存储拼音路径段（如 `seo-optimization`），不含 `/keyword/` 前缀和 `.html` 后缀。前台路由匹配时框架自动剥离前缀和后缀，控制器直接 `WHERE url_path = ?` 查询。

#### 5.2.5 xphp_task（定时任务）

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | int PK | 是 | 自增 | 主键 |
| name | varchar(50) | 是 | - | 任务名称 |
| type | varchar(30) | 是 | - | 任务类型（UNIQUE） |
| cron_desc | varchar(50) | 否 | - | 执行频率描述（仅展示，实际频率由外部 crontab 控制） |
| timeout | int | 否 | 300 | 超时时间（秒），用于 set_time_limit 和超时恢复 |
| last_run_time | int | 否 | 0 | 最后执行时间 |
| last_run_status | tinyint | 否 | 0 | 0 未执行 / 1 成功 / 2 失败 |
| last_run_msg | varchar(500) | 否 | - | 最后执行结果消息 |
| last_success_time | int | 否 | 0 | 最后成功执行时间 |
| total_run | int | 否 | 0 | 累计执行次数 |
| total_fail | int | 否 | 0 | 累计失败次数 |
| consecutive_fail | int | 否 | 0 | 连续失败次数（成功时重置为 0） |
| status | tinyint | 否 | 1 | 0 禁用 / 1 启用 |
| create_time | int | 否 | 0 | 创建时间（框架自动填充） |
| update_time | int | 否 | 0 | 更新时间（框架自动填充） |

#### 5.2.6 xphp_task_log（任务执行日志）

| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| id | int PK | 是 | 自增 | 主键 |
| task_id | int | 是 | - | 关联任务 ID（NOT NULL） |
| status | tinyint | 否 | 0 | 0 执行中 / 1 成功 / 2 失败 |
| result | text | 否 | - | 执行结果详情 JSON |
| start_time | int | 否 | 0 | 开始时间（0=未记录） |
| end_time | int | 否 | 0 | 结束时间（0=未记录） |
| duration | int | 否 | - | 耗时毫秒（由 microtime 差值计算） |
| create_time | int | 否 | 0 | 记录创建时间（框架自动填充） |

**索引**: `idx_task_log_task_start (task_id, start_time)`
...
| cron_key | Cron 安全密钥 | HTTP 触发方式安全验证 | 明文；自动生成 32 位随机字符 |
| cron_allowed_ips | Cron 允许 IP 列表 | IP 白名单限制；为空时默认拒绝所有 HTTP 触发 | 明文 |

### 5.4 后台菜单扩展

在原有 4 个系统菜单基础上新增 4 个业务菜单：

| 菜单名称 | 路由 | 图标 |
|----------|------|------|
| 关键词管理 | keyword/index | mdi mdi-key-variant |
| 页面管理 | page/index | mdi mdi-file-document-outline |
...
- 总超时控制：累计计时超过 `$timeout` 秒即终止重试并返回失败
...
**提示注入防护**: Prompt 模板中使用 `<keyword>{keyword}</keyword>` 标签标记关键词边界。关键词文本长度限制 100 字符。
...
**处理步骤**:
1. 移除空字节（`\x00`）和 Unicode 控制字符（U+200B-U+200F, U+202A-U+202E）
2. 循环 `html_entity_decode` 直到输出不再变化（最多 10 次）
3. 移除危险标签：script / iframe / embed / object / form / svg / math / details / applet / meta / link / base / template / noscript / portal / xmp
4. 保留 `<style>`，移除其中 `@import` / `expression()` / `url(javascript:)` / `behavior:` / `-moz-binding:`
5. 移除所有 `on*` 事件属性（大小写不敏感）
6. 移除 `javascript:` / `vbscript:` 协议（属性值中，先去除制表符/换行符后再检测）
7. 移除非 `<img>` 标签的 `data:` 协议（仅允许 `<img src="data:image/...">` 的 data URI）
8. 对所有保留标签的 `style` 属性执行与 `<style>` 相同的过滤规则
9. 如果内容不含 `<html>` 标签，自动包裹基本 HTML 结构：
```html
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
**Canonical URL**: 每个落地页必须输出 `<link rel="canonical" href="{site_url}/keyword/{url_path}.html">`，在 HTML 包裹时注入 head 区域。
...
| 编号 | 安全需求 | 实现方式 |
|------|----------|----------|
| SEC-001 | 前台只查 status=1，草稿不可访问，不存在返回 404 | Site::dispatch() 过滤 + 404 响应 |
| SEC-002 | HTML 纵深防御 + CSP 头 | filter_landing_html() 9步过滤 + CSP Header |
| SEC-003 | SSL 验证默认开启 | verify_ssl 默认值 1（Ollama 厂商预设为 0） |
| SEC-004 | 批量生成并发控制 | 单次最多3个，flock 互斥执行（手动+Cron 共享锁） |
| SEC-005 | 参数白名单校验 | direction 仅允许 related/question/longtail/commercial |
| SEC-006 | API Key 安全处理 | 新增必填/更新空值保留原值（Ollama 除外） |
| SEC-007 | Cron 安全防护 | 密钥验证（≥32位随机）+ 60秒限频 + flock锁 + IP白名单（空=拒绝所有） |
...
| SEC-019 | 资源限制 + 文件上传安全 | CSV 限制 2MB/1000行；MIME 类型验证；随机哈希重命名；上传目录禁止 PHP 执行；CSV 公式注入防护 |
...
| SEC-028 | 安全响应头 | Nginx 层配置（见 13.4 部署安全要求） |
...
| 步骤 | 任务 | 文件数 | 依赖 | 验证方式 |
|------|------|--------|------|----------|
| 1 | SQL 备份文件 | 修改3个+新增3个 | 无 | 执行安装器后检查 6 张新表 |
| 2 | 公共函数 | 1个文件追加7个函数 | 1 | PHP语法检查 |
| 3 | 模型 | 6个新文件 | 1,2 | PHP语法检查 |
| 4 | 后台控制器 | 4个新文件 | 1,2,3 | PHP语法检查 |
| 5 | 前台控制器 | 2个新文件 | 1,2,3 | PHP语法检查 |
| 6 | CLI 命令 | 1个新文件+新建目录 | 1,2,3 | `php xphpcli index.cron` |
| 7 | 路由配置 | 1个文件 | 无 | 访问测试URL |
| 8 | 视图模板 | 8个新文件+新建目录 | 1,4,7 | 浏览器渲染检查 |
| 9 | 修改现有文件 | 6个文件 | 2,3,7 | 功能集成检查 |
...
| `cache($name, $value, $expire)` | `xphp/helper.php` | 缓存：设置/获取/删除(null)/检测(?前缀) |
| `cache_make($name, $closure, $expire)` | `xphp/helper.php` | 缓存获取，不存在则创建 |
| `cache_clear($path)` | `xphp/helper.php` | 清除缓存 |
| `site($name, $default, $to_array=false)` | `xphp/helper.php` | 读取站点配置 |
| `db($table, $config=[])` | `xphp/helper.php` | 数据库查询构造器 |
| `model($name)` | `xphp/helper.php` | 模型实例化，**分隔符为 `@`** |
| `pdo($config)` | `xphp/helper.php` | PDO实例，`pdo()->trans(fn)` 事务 |
| `halt($msg, $code, $params=[])` | `xphp/helper.php` | 中断输出（never） |
| `ids_filter($ids, $to_array, $gt_0=true)` | `xphp/function.php` | ID过滤转换 |
| `name_parse($name, $app='', $sep='@')` | `xphp/function.php` | 名称解析 |
| `remove_xss($str)` | `xphp/function.php` | XSS过滤（会移除style标签，不适用于落地页） |
| `get_curl($url, $post, ...)` | `xphp/function.php` | cURL请求（不支持JSON body和SSL验证控制） |
| `get_ip()` | `xphp/function.php` | 获取客户端IP |
| `ROOT_PATH` | `public/index.php` | 项目根目录 |
| `RUNTIME_PATH` | `xphp/core/App.php` | 运行时目录 |
| `__HOST__` | `xphp/core/App.php` | 当前协议+域名 |
| `__ROOT__` | `xphp/core/App.php` | 站点根路径 |
| `__STATIC__` | `xphp/core/App.php` | 静态资源路径 |

### 10.2 关键约束

- `Model::save()` 和 `Model::del()` 均为 `final`，不可覆写；关联完整性检查须在控制器层执行
- 公共文件路径使用 `ROOT_PATH . '/public/'` 拼接
- `site()` 函数读取 `app/index/config/site.php`，config 表变更通过 `syncConfigFile()` 自动同步
- 现有 config 中 `site_title` 用于后台标题栏，新增 `site_name` 用于落地页生成，二者为不同字段

### 10.3 踩坑记录

**坑1：`model()` 函数分隔符是 `@` 不是 `.`**
- ✅ 正确：`model('common@keyword')`、`model('admin@ai_config')`
- ❌ 错误：`model('common.keyword')` — 不会报错但模型无法正确加载

**坑2：PHP 8.1+ 子类方法签名必须兼容父类（LSP 严格检查）**
- Cp 基类签名：`del(string $ids)`、`edit(int $id, array $req)`、`state(string $ids, string $params)`
- 覆盖时参数数量和类型必须完全一致，否则 Fatal Error 导致整个类无法加载
- **症状**：HTTP 200 空白响应（0字节），`php -l` 无法检测此错误
- **调试方法**：`php -r "new ReflectionClass('app\admin\controller\Keyword')"` 来触发错误

**坑3：分页渲染必须用 `{$list->links()|raw}`，不能用 `{$list|raw}`**
- ✅ 正确：`{$list->links()|raw}`
- ❌ 错误：`{$list|raw}` — 会触发 Query 对象转字符串错误
- 判断空列表用 `{empty $list->toArray():}` 而非 `{empty $list:}`

**坑4：`db()` 使用 JOIN + `paginate()` 会导致 count() 查询失败**
- ✅ 解决方案：避免在 `paginate()` 中使用 JOIN，改用分步查询

**坑5：后台视图必须包含完整 lyear 布局**
- 必须包含：`_head.html` → `lyear-preloader` → `lyear-layout-web` → `sidebar.html` → `_header.html` → `<main>` → `footer.html`

**坑6：模板修改后必须清除编译缓存**
- 删除 `runtime/admin/view/` 下所有文件

**坑7：`APP_DEBUG=false` 时错误被完全隐藏**
- 调试时临时开启 `config/app.php` 的 `debug => true` 和 `trace => true`
...
{
    protected string $model = 'admin@config';
...
**模型文件位置**: 跨应用模型放在 `app\common\model\` 下，使用 `model('common@keyword')` 引用；同应用模型放在 `app\admin\model\` 下，使用 `model('admin@ai_config')` 引用。

**事务内模型操作**: 框架 `pdo()->trans()` 仅在闭包抛异常时回滚。模型 `save()` 验证失败时调用 `halt()` 直接 exit。事务内应手动检查 `isFail()` 并抛异常：
```php
pdo()->trans(function() {
    $model->save($data);
    if ($model->isFail()) {
...
**Jump Trait**（用于不继承 Cp 的控制器）:
```php
class Task { use Jump; protected string $jumpUrl = 'index'; ... }
```
...
- 列表操作：`ajaxConfirm(url, title)` / `actionConfirm(title, url)` / `openModal(url, title)`
...
### 11.3 新增 `3_insert_xphp_ai_config_part1.sql`
...
### 11.4 新增 `3_insert_xphp_prompt_part1.sql`
...
### 11.6 修改 `3_insert_xphp_config_part1.sql`
...
### 11.7 修改 `3_insert_xphp_menu_part1.sql`

替换整个文件内容（8条菜单：4系统+4业务）：
...
文件：`app/admin/model/AiConfig.php`
...
class Keyword extends Model
{
    ...
    public function expandByAi(int $keywordId, string $direction): array
    {
        ...
        $promptText = render_prompt($promptText, ['{keyword}' => $keyword['word']]);
...
引用方式：`model('common@page')`。`_clearViewCache` 为 public，Page 控制器 `state()` 方法需调用。
...
class Task extends Model
{
    ...
    public function execPushBaidu(): string
    {
        ...
        if (($result['success'] ?? 0) > 0) {
            db('page')->where('status', 1)->where('is_pushed_normal', 0)->setField('is_pushed_normal', 1);
        }
    ...
    public function execPushBaiduFast(): string
    {
        ...
        if (($result['success'] ?? 0) > 0) {
            db('page')->where('status', 1)->where('is_pushed_fast', 0)->where('is_pushed_normal', 0)->setField('is_pushed_fast', 1);
        }
    ...
    public function execSitemap(): string
    {
        ...
        foreach ($pages as $page) {
            ...
            $xml .= "  <url>\n";
            $xml .= "    <loc>" . $siteUrl . "/keyword/" . $page['url_path'] . ".html</loc>\n";
        ...
        file_put_contents($publicPath . 'robots.txt', $robotsContent);
    ...
    public function execClearCache(): string
    {
        ...
        $deletedLogs = db('task_log')->where('create_time', '<', $expire)->delete();
...
AI引擎菜单入口控制器，同时管理 AI配置 和 提示词模板（选项卡切换）。
...
class AiConfig extends Cp
{
    ...
    public function test()
    {
        ...
        $lockKey = 'ai_test_' . $id;
        if (cache('?' . $lockKey)) $this->_json(429, '请30秒后再试');
        cache($lockKey, 1, 30);

        $config = db('ai_config')->find($id);
        if (!$config) $this->_json(400, '配置不存在');
    ...
    public function promptSave()
    {
        ...
        if ($id) {
            ...
            if (!$prompt) $this->_jump([null, '提示词不存在'], false, url('index') . '?tab=prompt');
            $r = $prompt->save($data);
        } else {
    ...
    public function promptDel()
    {
        ...
        if (empty($idArr)) $this->_json(400, '请选择要删除的记录');
        $count = 0;
        foreach ($idArr as $id) {
...
class Keyword extends Cp
{
    ...
    public function import()
    {
        if ($this->isPost()) {
            ...
            if (empty($file) || !preg_match('/^[a-zA-Z0-9_\-\.]+$/', $file)) $this->_json(400, '文件名无效');
    ...
    public function generate()
    {
        ...
        try {
            foreach ($keywords as $kid) {
                ...
                if ($r) { $progress['done']++; } else { $progress['failed']++; }
    ...
    public function progress()
    {
        ...
        if (empty($batchId)) $this->_json(400, '参数错误');
        $progress = cache('seo_generate_progress_' . $batchId);
        if (!$progress) $this->_json(400, '任务不存在');
...
class Page extends Cp
{
    ...
    public function index()
    {
        ...
        foreach ($list->data as &$item) {
            if (!empty($item['keyword_id'])) {
                $keyword = db('keyword')->where('id', $item['keyword_id'])->find();
                $item['keyword_word'] = $keyword['word'] ?? '';
            } else {
                $item['keyword_word'] = '';
            }
        }
    ...
    public function state(string $ids, string $params)
    {
        ...
        if ($field === 'status' && $value == 1) {
            db('page')->where('id', 'in', $idArr)->update(['is_pushed_normal' => 0, 'is_pushed_fast' => 0]);
        }
        if ($field === 'status') {
            cache('sitemap_dirty', true);
        }
...
class Task
{
    ...
    public function state(string $ids, string $params)
    {
        ...
        [$field, $value] = name_parse($params, 'status', '-');
        $map = [[$field, '<>', $value]];
        if (count($ids) == 1) { $map['id'] = current($ids); } else { $map[] = ['id', 'in', $ids]; }
...
class Site
{
    ...
    public function dispatch(string $path = '')
    {
        ...
        $relatedPages = db('page')->where('status', 1)->where('id', '<>', $page['id'])->field('url_path,title')->order('id DESC')->limit(5)->select();
        $siteUrl = site('site_url') ?: __HOST__;
        $relatedHtml = '';
        if (!empty($relatedPages)) {
            $relatedHtml .= '<div class="related-pages"><h3>相关推荐</h3><ul>';
            foreach ($relatedPages as $rp) {
                $relatedHtml .= '<li><a href="' . $siteUrl . '/keyword/' . $rp['url_path'] . '.html">' . htmlspecialchars($rp['title']) . '</a></li>';
            }
            $relatedHtml .= '</ul></div>';
        }
        ...
        return view()->with('content', $content)->cache(3600);
    }
}
```
...
<?php
return [
    'keyword/([a-zA-Z\x7f-\xff0-9-%\+]+)' => 'site/dispatch/keyword/${1}',
    'cron/([a-zA-Z0-9\-_]+)' => 'cron/run/key/${1}',
];
```

| 路由 | 参数别名 | 正则 | 说明 |
|------|----------|------|------|
| `keyword/:keyword` | `:keyword` | `[a-zA-Z\x7f-\xff0-9-%\+]+` | 支持中文 |
| `cron/:string` | `:string` | `[a-zA-Z0-9\-_]+` | 限制密钥格式为字母数字及连字符下划线 |

框架 `url_clear_suffix => ['.html']` 自动去除后缀。
...
| 文件 | 说明 |
|------|------|
| `app/admin/view/ai_config/index.html` | AI引擎入口：顶部选项卡（AI配置/提示词模板）+ AI配置列表（统计卡片3列+列表+Modal表单+厂商下拉自动填充+测试连接） |
| `app/admin/view/ai_config/prompt.html` | 提示词模板：顶部选项卡+类型筛选+列表+Modal表单+激活按钮（二次确认） |
| `app/admin/view/keyword/index.html` | 关键词管理：搜索+筛选+工具栏+AI拓词弹窗+批量生成进度条（3秒轮询，最大5分钟，连续3次请求失败停止） |
| `app/admin/view/page/index.html` | 页面管理：筛选+列表+新增Modal+AI重写（loading+确认框）+状态切换 |
| `app/admin/view/page/edit.html` | 页面编辑：上方表单+下方textarea代码编辑（等宽字体，最小高度500px）；已发布url_path只读；AI重写按钮（确认框） |
| `app/admin/view/task/index.html` | 定时任务：列表+Modal表单（含超时时间配置）+手动执行+日志链接 |
| `app/admin/view/task/logs.html` | 任务日志：列表+失败项红色标记+result JSON 展开 |

**后台视图开发规范**:
1. 必须包含完整 lyear 布局（`_head.html` → `lyear-preloader` → `lyear-layout-web` → `sidebar.html` → `_header.html` → `<main>` → `footer.html`）
2. 分页用 `{$list->links()|raw}`，判断空用 `{empty $list->toArray():}`
3. 模板修改后清除 `runtime/admin/view/` 编译缓存

**选项卡实现**：AI配置选项卡链接 `{:url('index')}`，提示词模板选项卡链接 `{:url('index')}?tab=prompt`，当前页高亮 `active` 类。
...
追加 7 个函数：`ai_chat()`、`filter_landing_html()`、`to_pinyin()`、`generate_url_path()`、`baidu_push()`、`parse_seo_meta()`、`render_prompt()`（详见第 6 节）

### 16.2 `route/index.php`

替换返回数组（详见第 14 节）

### 16.3 `app/index/controller/Index.php`
...
### 16.4 `template/default/index/index.html`

在公告栏下方追加卡片式页面列表（标题+摘要+时间，最多10条，不分页）。页面发布/下线/删除时清除 `cache('index_pages', null)`。

### 16.5 `app/admin/controller/Index.php`
...
### 16.6 `app/admin/view/index/index.html`
...
2. **配置提示词模板** — 进入 AI引擎 → 提示词模板，5条默认模板已内置，可按需编辑内容，激活所需模板
3. **配置站点信息** — 进入后台 → 网站配置，填写网站名称、网站URL、百度站点域名和推送 Token（如需百度推送）
4. **添加关键词并生成页面** — 进入后台 → 关键词管理 → 添加关键词 → 批量生成页面（生成结果为草稿，需手动发布）
5. **配置定时任务** — 进入后台 → 定时任务 → 启用所需任务，配置 crontab: `* * * * * cd /path && php xphpcli index.cron`
