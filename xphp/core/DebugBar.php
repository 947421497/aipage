...
class DebugBar
{
    ...
    {
        $trace = $this->trace;
        return '<!--By XPHP.NET Framework | Processed in ' . $trace['time'] . ', Memory ' . $trace['memory'] . ', ' . count($this->items['sql']) . ' queries, ' . $trace['total'] . ' files(' . $trace['filesize'] . ')-->';
    }
    ...
    protected function getDebugBarHtml(): string
    {
        ...
        ob_start();
        include ROOT_PATH . '/xphp/tpl/debug_trace.php';
        return "\n" . ob_get_clean() . "\n";
    }
    ...
    protected function parseTrace(): array
    {
        ...
        $trace['base'] = [
            ...
            '运行时间：' . $this->trace['time'] . ' at ' . date('Y-m-d H:i:s', $_SERVER['REQUEST_TIME']) . ' <a href="https://xphp.net" style="color:green;" target="_blank" rel="noopenner noreferrer">XPHP v' . __VERSION__ . '</a>',
        ];
        $list = [];
        foreach ($tabs as $k => $v) {
            $list[$k] = $trace[$k];
