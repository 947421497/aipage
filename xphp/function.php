...
// 检测应用名，不通过返回默认index
function parse_app_name(string $name, string $default = 'index'): string
{
    if (empty($name)) {
        return $default;
    }
    return preg_match('/^[A-Za-z_]+$/', $name) ? strtolower($name) : $default;
}
...
function name_parse(string $name, string $app = '', string $sep = '@'): array
{
    ...
    if (str_contains($name, '.')) {
        [$tmp, $name] = explode('.', $name, 2);
        $app = parse_app_name($tmp, $app);
    }
    return [$app, $name];
}
