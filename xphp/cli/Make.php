...
class Make extends Command
{
    ...
    public function cli(): bool
    {
        if (!$this->isCall) {
            ...
            echo "| 6. make:app     [app_name] [entry_file:'']                                 |\n";
    ...
    public function table(array $req = []): ?bool
    {
        ...
        if (!empty($sql)) {
            ...
            $pdo = pdo();
            $hasTable = $pdo->hasTable($replace['table_name']);
            if ($hasTable) {
                if (!$is_cover) {
                    return $this->error('表已存在');
                }
                $pdo->execute('DROP TABLE IF EXISTS `' . $replace['table_prefix'] . $replace['table_name'] . '`;');
            }
            $pdo->execute($sql);
    ...
    // 生成应用：make:app 应用名 入口文件entry
    public function app(array $req = []): ?bool
    {
        $app = parse_app_name($req[0] ?? '');  // 应用名
        $entry = parse_app_name($req[1] ?? '', ''); // 入口文件
        $namespace = 'app\\' . $app; // 命名空间
        ...
        if (!empty($entry)) {
            // 生成入口文件
            $file = ROOT_PATH . '/public/' . $entry . '.php';
            if (!is_file($file)) {
                $content = file_get_contents(ROOT_PATH . '/xphp/cli/make/app/_index.tpl');
                $this->replace = ['app' => $app];
    ...
    protected function _get_tpl_file(string $app, string $tpl, string $type): string
    {
        $tpl_list = [
            ...
            ROOT_PATH . '/xphp/cli/make/' . $type . '/' . $tpl . '.tpl',
        ];
        ...
        return ROOT_PATH . '/xphp/cli/make/' . $type . '/_def.tpl';
    }
